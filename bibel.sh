#!/bin/sh
# bibel: Read the Word of God from your terminal
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data bibel.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data bibel.awk)" | ${PAGER}
	done
	exit 0
fi

get_data bibel.tsv | awk -v cmd=ref -v ref="$*" "$(get_data bibel.awk)" | ${PAGER}
